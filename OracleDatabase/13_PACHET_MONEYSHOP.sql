-- =====================================================
-- PACHET PKG_MONEYSHOP
-- Proiect SBD - Cerința 9.7
-- =====================================================
...
CREATE OR REPLACE PACKAGE PKG_MONEYSHOP AS
    ...
    -- Procedură care declanșează triggeri LMD prin INSERT
    ...
    -- Funcție helper pentru inserare mesaje
...
CREATE OR REPLACE PACKAGE BODY PKG_MONEYSHOP AS
    ...
    -- Tipuri de colecții (pentru procedura cu colecții)
    ...
    -- Procedură cu colecții (din cerința 9.1)
    PROCEDURE SP_PROCESARE_COLECTII (
        p_id_utilizator IN NUMBER,
        p_rezultat OUT VARCHAR2
    )
    ...
    -- Procedură cu cursoare (din cerința 9.2)
    PROCEDURE SP_PROCESARE_CURSOARE (
        p_id_rol IN NUMBER,
        p_rezultat OUT VARCHAR2
    )
    ...
    -- Funcție cu 3 tabele și excepții (din cerința 9.3)
    FUNCTION FN_CALCUL_STATISTICI_UTILIZATOR (
        p_id_utilizator IN NUMBER
    ) RETURN VARCHAR2
    IS
    ...
    -- Procedură care declanșează trigger LMD la nivel de linie (UPDATE)
    PROCEDURE SP_ACTUALIZARE_APLICATIE (
        p_id_aplicatie IN NUMBER,
        p_nou_status IN VARCHAR2,
        p_suma_aprobata IN NUMBER DEFAULT NULL
    ...
    BEGIN
        ...
        -- Această instrucțiune va declanșa triggerul trg_aplicatii_updated (LMD la nivel de linie)
        UPDATE APLICATII
    ...
    -- Procedură care declanșează trigger LMD la nivel de linie (INSERT)
    PROCEDURE SP_CREARE_APLICATIE (
        p_id_utilizator IN NUMBER,
        p_type_credit IN VARCHAR2,
        p_tip_operatiune IN VARCHAR2,
    ...
    BEGIN
        ...
        -- Această instrucțiune va declanșa triggerul trg_utilizatori_varsta (dacă e cazul)
        -- și triggerul pentru UpdatedAt
    ...
    EXCEPTION
        WHEN OTHERS THEN
            -- Dacă tabelul MESAJE nu există, returnăm 0
            RETURN 0;
